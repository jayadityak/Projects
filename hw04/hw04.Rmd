...
ord = ord %>% 
  ...
  mutate(month_day = day(date)) %>% 
  mutate(week_day = as.character(wday(date,label = TRUE,abbr = TRUE)))%>%
  relocate(year, month, month_day, week_day, .after = date) %>% 
  select(-terminal) %>% 
...
```{r}
all_data = expand_grid(date = seq(ymd("2017-01-01"), ymd("2021-12-31"), 1))
ord_data = ord %>% 
  select(airport, date) %>% 
  distinct() 
all_data %>% 
...
ord = ord %>% 
  mutate(time_of_day = case_when(
    ...
    (str_detect(hour, "1600 - 1700|1700 - 1800|1800 - 1900|1900 - 2000") ~ 'early afternoon'), 
...
prob3 = ord %>% 
  ...
  summarize(number_flights = sum(flights))
prob3
```
...
```{r}
ggplot(prob3, aes(x = time_of_day, y = number_flights))+
  geom_col()+
  xlab("Time of Day")+
...
## data summary
prob5 = ord %>% 
  select(month,year,passengers) %>% 
  group_by(month,year) %>% 
  summarize(AVERAGE = mean(passengers))
prob5
...
## plot the table  
ggplot(prob5, aes(x = month, y = AVERAGE)) +
  geom_col() +
  xlab("Average Number of Passengers")+
  ylab("MONTH")+
  ggtitle("Average Number of Passenger each Month for Given years")+
...
> The chart of 2020 is different from the other 4 charts. There were less number of passengers in 2020 because of the situation of covid-19 and lockdown. The months ranging from april to december had comparatively less number of passengers.
...
ord1 = ord %>% 
  ...
  summarize(passenger_sum = sum(passengers),
            day = n())
ord_avg = ord1 %>% 
  mutate(average_per_day = passenger_sum/day) %>% 
  arrange(desc(average_per_day))
ord_avg
```
...
```{r}
ord2 = ord %>% 
  select(date, week_day, passengers) %>% 

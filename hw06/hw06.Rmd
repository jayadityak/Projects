...
prob_1 %>% 
  summarize(n = n(), first = min(color), last = max(color))
...
```{r}
prob_2 = prob_1 %>% 
  filter(str_detect(color,"\\d",negate = TRUE)) %>% 
  summarize(n=n(),min(color),max(color))
prob_2
...
```{r}
prob_4 = df_colors %>% 
  filter(str_detect(color, "([a-z])\\1"))

prob_4 %>% 
  slice(1:10)

prob_4 %>% 
  group_by(consecutive = str_extract(color,"([a-z])\\1")) %>% 
  summarize(color, count = n()) %>% 
  filter(count == 1)
...
> They all have punctuation marks in their names which are considered as regular expressions.
...
prob_7 = planets %>%
  ...
  mutate(star_regex = str_replace_all(star_regex, "\\+", "\\\\+")) %>%
  filter(str_detect(planet, star_regex)) %>% 
  mutate(last = str_sub(planet, -1, -1)) %>% 
  count(last)
prob_7 

ggplot(prob_7, aes(x = last, y = n)) +
  geom_col(color = 'black', fill = 'blue') +
  xlab('Last lower case letter') +
  ylab('Count') +
  ggtitle('Distribution of Last lower case letter of planet')
```
...
prob_8 = wrdl %>% 
  filter(str_count(word, "t") > 0 &
         str_count(word, "c") > 0 &
         !str_detect(word, "[slaeun]")) %>% 
  filter(str_detect(word, "^[^c]o.[^t][^t]$"))
prob_8
```

...
ggplot(monona, aes(x = duration)) +
  geom_histogram(boundary = 70 , binwidth = 5 ,
                 color = "forestgreen" , fill = "cyan") +
  xlab("Days Closed with Ice") +
  ylab("Total") +
  ggtitle("Lake Monona Freeze Durations, 1855-2022")
...
The geom_smooth function reduces the noise and replaces the actual data values with a smooth curve and the argument (se=FALSE) removes the uncertainty in the trend line while estimating values.
...
There is a negative correlation between the year and duration, As the year increases, the duration slightly decreases.
...
ggplot(monona, aes(y=duration)) +
  ...
  ylab("Days Lake is Closed by Ice") + 
...
ggplot(monona, aes(x=ff_cat)) +
  ...
  xlab("Time of First Freeze") +
  ylab("Number of winters") + 
  ggtitle("First Freeze Durations",
          subtitle ="1855-2021")
...
Because we're only considering 1 variable here and the heights of the variable need to be calculated (as the count)
...
ggplot(monona, aes(x = ff_x, y = duration, color = period50)) +
  ...
  ggtitle("First Freeze vs Duration")
...
The number of days that would freeze would be greater (this can be seen by finding the y-value for the same x-values for both time durations)
...
Faceting, because it makes it easier to visualize.
